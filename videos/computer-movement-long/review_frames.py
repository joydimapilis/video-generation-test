...
manifest={'source':str(a.video),'hand_fps':a.fps,'full_face_fps':2,'method':'chronological sampled frames, not continuous real-time playback','sheets':[],'crops':crops}
for kind,(x,y,x2,y2) in crops.items():
 folder=a.out/kind;folder.mkdir(exist_ok=True)
 sample_fps=a.fps if kind=='hands' else 2
