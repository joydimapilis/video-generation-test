...
for note in [50,57,62,66]: place(pluck(note+12,3),27,.09)
for start in [7.2,12.3,22.6,26.0]: place(pluck(86,.35),start,.035)
t=np.arange(len(audio))/SR
envelope=np.minimum(t/.12,1)*np.minimum(np.maximum((30-t)/1.6,0),1)
audio*=envelope[:,None]
...
(ROOT/'audio_meta.json').write_text(json.dumps({'bgm':{'path':'assets/score.wav','volume':1},'voices':[],'sfx':[],'bgm_pending':False,'provenance':'Original local deterministic composition: make_score.py; no samples, no API spend; 112 BPM; attack and final fade baked in'},indent=2))
(ROOT/'audio-review.json').write_text(json.dumps({'duration_seconds':30,'sample_rate':SR,'channels':2,'peak_dbfs':round(20*np.log10(np.max(np.abs(audio))),2),'rms_dbfs':round(20*np.log10(np.sqrt(np.mean(audio**2))),2),'clipped_samples':int(np.sum(np.abs(audio)>=1)),'first_five_seconds_rms_dbfs':round(20*np.log10(np.sqrt(np.mean(audio[:5*SR]**2))),2),'last_second_rms_dbfs':round(20*np.log10(np.sqrt(np.mean(audio[-SR:]**2))),2),'review_limit':'Numerical waveform and synthesis review, not independent perceptual listening. No narration or speech sync claims.','local_cost_usd':0},indent=2))
print(out)
