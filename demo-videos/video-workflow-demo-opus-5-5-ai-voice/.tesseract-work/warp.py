...
for seg, tok in PINS:
    try:
        pins.append((first(old_w, seg, OLD_ALIAS.get(tok, tok))["a"], first(new_w, seg, tok)["a"]))
...
man_gain = 10 ** (8.1 / 20)  # processed Kokoro sentences: -25.8 LUFS mono, +3 LU once centred in stereo
audio = []
