...
Pauses copy the original edit's pacing: ~0.45 s between sentences inside a
chapter, and the original gaps between chapters. The Kite video plays unchanged.
"""
...
FIRST_WORD = 0.51
GAP_IN = 0.45
GAP_AFTER = {"V1-1": 0.25, "V4b-1": 0.75,  # sentence-level overrides
...
def words_from_alignment(fn, aln_path):
    """Word timings from ElevenLabs' character alignment (letters only; punctuation ignored),
    with the first/last word clamped to the measured speech extent of the decoded audio."""
    a = json.load(open(aln_path))["alignment"]
    words, cur = [], None
    for c, st, en in zip(a["characters"], a["character_start_times_seconds"], a["character_end_times_seconds"]):
        if c.isspace():
            if cur: words.append(cur); cur = None
            continue
        if cur is None: cur = {"w": c, "a": st, "b": en if c.isalnum() else st}
        else:
            cur["w"] += c
            if c.isalnum() or c == "'": cur["b"] = en
    if cur: words.append(cur)
    s0, s1 = speech_extent(fn)
    words[0]["a"] = max(words[0]["a"], s0)
    words[-1]["b"] = min(max(words[-1]["b"], words[-1]["a"] + 0.1), s1)
    return words, s0, s1
...
def build():
    ...
    for n, s in enumerate(sents):
        ...
        aln = os.path.join(ROOT, s["alignment"]) if s.get("alignment") else None
        words, s0, s1 = words_from_alignment(fn, aln) if aln else words_for(fn)
        if t is None:
            start = FIRST_WORD - s0
        else:
            start = t
        ...
        gap = GAP_AFTER.get(f"{s['key']}-{s['idx']}", GAP_AFTER.get(s["key"], GAP_IN) if last_in_step else GAP_IN)
