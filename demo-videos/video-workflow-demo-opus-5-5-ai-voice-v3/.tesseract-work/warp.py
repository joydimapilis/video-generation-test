...
NEW_ALIAS = {"reverse": ["reverse", "reverseengineering"]}  # ElevenLabs keeps "reverse-engineering" as one word
pins = []
for seg, tok in PINS:
    try:
...
man_gain = 1.0  # processed ElevenLabs sentences already carry their gain (see Assets/SOURCES.md)
audio = []
...
say_norm = lambda t: [norm(x) for x in t.split() if norm(x)]  # AI words come from the script text itself (ElevenLabs alignment)
cap_tokens, cap_of = [], []
